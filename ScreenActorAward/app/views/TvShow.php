...
<a href="http://localhost/ScreenActorAward/app/views/Seasons.php?show_id=<?= isset($tvShow['id']) ? $tvShow['id'] : '1' ?>" class="view-more">View All Seasons</a>
        </div>
</div>
...
function toggleSearchBar() {
    ...
    if (tvShow && tvShow.cast) {
        ...
        tvShow.cast.forEach(item => {
            ...
            const actorPageUrl = `http://localhost/ScreenActorAward/public/actor/${encodeURIComponent(item.name)}`;
            castItem.innerHTML = `
                <a href="${actorPageUrl}">
