                    ...
                    <?php foreach ($search as $result): ?>
                        <div class="search-result-item" data-type="<?= htmlspecialchars($result['media_type']); ?>">
                            ...
                            if ($result['media_type'] === 'movie') {
                                ...
                                $detailLink = "http://localhost/ScreenActorAward/app/views/Actor.php?name=" . htmlspecialchars($result['name']);
                            }
                            ?>
                            <a href="<?= $detailLink; ?>">
                                <?php if (isset($result['poster_path']) || isset($result['profile_path'])): ?>
                                    <img src="https://image.tmdb.org/t/p/w185<?= htmlspecialchars($result['poster_path'] ?? $result['profile_path']); ?>"
                                        alt="<?= htmlspecialchars($result['name'] ?? $result['title']); ?>">
                                <?php else: ?>
                                    <img src="http://localhost/ScreenActorAward/public/pictures/default-poster.jpg"
                                        alt="Default Poster">
                                <?php endif; ?>
                            </a>
